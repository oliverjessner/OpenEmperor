...
The separate `--view stored-graphics` command requires `--graphics-profile exe-6373328b-v213-runtime-table` and no binding file. It reads the saved 32-bit word at every candidate storage cell and resolves only the studied v213 Terrain/Elevation slots. It draws a supported single Emperor Type-30 footprint when width is 78, height is at least 40, the base is 3,200 bytes, source ranges are valid, and mirroring is absent. The full decoded image, including any Omega overlay, is drawn at preview origin `world − (width/2, height−40)`; this is a diagnostic placement, not a reconstructed original-engine rule or terrain height. Other layouts and failed decodes remain purple status-marked cells in the same depth order. The view checks source paths under `--data`, permits at most 2,048 distinct referenced assets, 16 MiB logical RGBA per image, and 64 MiB total logical RGBA textures. It decodes and uploads each distinct supported asset once, never per frame. Selection prints saved ID, source offset, slot/local/physical record, metadata, bounds and decode status. No later graphics recalculation, multi-cell placement, or full original first frame is implied. See [stored graphics preview](docs/stored-graphics-preview.md).
