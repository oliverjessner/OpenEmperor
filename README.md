...
The opt-in `exe-6373328b-v213-slot8-runtime-table` profile also registers slot 8 as `DATA/China_Mon_Earthen_Greatwall_1.sg3` for maps that reference it. This registration is backed by the same hash-checked v213 resource-manager path and is separate from the unchanged Terrain/Elevation base profile. The archive's large Type-30 Great Wall footprints remain diagnostic under the existing placement rules; resolving a saved ID does not mean it can be displayed. The local evidence and limits are in [graphics-ID notes](docs/reverse/graphics-id.md).
