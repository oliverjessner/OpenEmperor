...
./build/openemperor-map-graphics --data /path/to/your/game-data --group-key 0x603 --variants 8
./build/openemperor-map-graphics --data /path/to/your/game-data --group-key 0x603 --variants 8 --map Cities/Xia.map --cell 114 114 --cell 115 114
./build/openemperor-map-graphics --data /path/to/your/game-data --map Cities/Xia.map --terrain-selection-profile exe-6373328b-first-terrain-probe --cell 114 114 --cell 115 114
...
The optional `openemperor-map-graphics --profile exe-6373328b-14bit-hypothesis` diagnoses one version-specific graphics-ID interpretation found by bounded static analysis of a locally supplied EXE. It keeps the original raw candidate and the rejected direct-index H1 report, and separately reports a 14-bit slot/local split against explicitly named Terrain/Elevation archives, metadata and payload gates, and selected-image decodes. The observed v213 Terrain/Elevation loader skips the dummy SG3 record: runtime local index `i` selects physical SG3 record `i+1`, bounded by the header's reported in-use count. The diagnostic now reports both indices, the physical SG3 record offset, and separate evidence flags. The map-load path copies the stored candidate-word range into the per-cell array that feeds a related graphic lookup. Later game routines can replace those values, so the exact value at an original-game draw and its visual match remain unverified. The 4×9 sprite and 78×41 image in the selected demonstration do not establish flat map-tile placement; this profile therefore does not drive the SDL map preview. For a local diagnostic image, run `./build/openemperor --sg3 /path/to/China_Terrain.sg3 --image 40` after confirming the physical index in the tool's output. See [graphics-ID evidence](docs/reverse/graphics-id.md) for addresses, limits, and the four-map check.

The separate `--terrain-selection-profile exe-6373328b-first-terrain-probe` compares selected simple-ground cells with their stored graphic IDs and conditional SG3 decode. It deliberately reports no computed ID: the observed range writer runs before the stored map graphic array is read, and the path from those saved values to the first draw is not yet complete. It does not alter the curated preview or imply a recovered terrain rule.

The explicit `--group-key 0x603 --variants 8` query independently resolves one observed resource-group lookup for the studied EXE and v213 Terrain registration. The SG3 index word at file offset 186 yields packed base `0xc557`; eight supplied variant values decode physical Terrain records 1368–1375. Optional `--map`/`--cell` arguments compare that base with untouched saved graphic IDs and a separately read byte layer. The sampled Xia, Banpo, Chengdu, and Anyi cells do not match the group's 0–7 range. This group query does not select a terrain image for loaded-map rendering or reproduce the original generator state. See [graphics-ID evidence](docs/reverse/graphics-id.md).
