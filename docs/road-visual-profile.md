...
Roads, buildings and walkers share the existing projected-ground painter order, with Road before Building before Walker at the same ground key. The saved-original-map `StoredGraphicsRenderer` remains a separate earlier background pass; original tall objects cannot yet occlude sandbox roads or buildings correctly. The F6 debug line reports ON/OFF, configured masks, unique assets and fallbacks in the current frame. The Industry-v5 `--sandbox-check --sandbox-resume-check --report-json` path adds `road_visuals` counts and World equality to a separately ticked control; it always leaves `manual_visual_review` false.
