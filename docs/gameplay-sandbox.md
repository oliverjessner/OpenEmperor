...
Keys `1`–`6` select Road, Clay source, Pottery, Warehouse, inspection, and Remove road. The existing pause, step, speed, pan, zoom, reset, mouse, and Escape controls still apply. SDL primitive geometry distinguishes the three one-cell buildings, courier A (Clay) and courier B (Pottery), including their loads. A waiting courier is orange while its cargo indicator remains visible. The HUD reads source output/progress, pottery input/active recipe/progress/output, both courier states, warehouse stock/reservations, road revision, and the two conservation totals from the simulation. It has no timer or inventory of its own.
...
Existing valid `schema_version: 1` and `schema_version: 2` files remain readable under their original profiles. Logistics rules stay at `rules.version: 1`. A known old production schema-1 save with `rules.version: 1` is validated, given zero road-removal and reroute counters, and upgraded **in memory** to production `rules.version: 2`; schema-2 production retains version 2. Neither becomes the household profile. Loading does not advance a tick or rewrite the original save. A subsequent explicit F5 writes schema 3. Unknown schema and rule versions still fail.
...
Keys `1`–`6` keep the v2 tools; `7` places the only Household. The green household diamond and third supplier marker are distinct from existing buildings and couriers. The HUD reports house stock (capacity 8), incoming reservation, demand-clock progress, last demand result, fulfilled and missed dates, total Pottery consumed, supplier phase/cargo, and connection status. Pause and single-step use the same fixed tick driver; F5/F9 save and restore the complete household clock and any in-flight or waiting delivery.
