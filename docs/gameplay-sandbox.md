...
Existing valid `schema_version: 1` files remain readable. Their unchanged logistics rules stay at `rules.version: 1`. A known old production save with `rules.version: 1` is validated, given zero road-removal and reroute counters, and upgraded **in memory** to the new production `rules.version: 2`. Loading does not advance a tick or rewrite the original save. A subsequent explicit F5 writes schema 2. Unknown schema and rule versions still fail.
