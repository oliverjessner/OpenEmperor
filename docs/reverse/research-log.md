...
- A local smoke test using ignored GOG data decoded a 32×32 Type-256 Sprite from `China_Mon_Tumulus.sg3` at index 2 and displayed it in SDL3; Escape closed the window normally. The temporary visual check was deleted. No proprietary bytes or derived image files were added to the repository.
...
- A local ignored `China_Mon_Tumulus.sg3` has 376 Type-30 records. Index 203 decoded and visibly rendered as a 78×40 single Emperor tile (derived footprint N=1, no overlay). Index 371 decoded and visibly rendered as a 158×80 four-tile Emperor footprint (derived N=2, no overlay). Each app window exited normally with Escape. Index 201, a 78×41 image with nonempty overlay, also decoded through the application loading path; that check did not open a display in the sandbox. Temporary visual checks were deleted.
- Alpha-mask decoding, applying the parsed mirror offset, the SG3 suffix, and observed out-of-bounds source ranges remain unresolved. No offset correction, alpha interpretation, map logic, or game simulation is introduced.
