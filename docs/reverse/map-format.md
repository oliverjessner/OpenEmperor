...
## Saved-ID 2×2 observation (2026-09-17)

A bounded read of `Cities/Chengdu.map` at storage `(55,133)`, `(56,133)`, `(55,134)`, and `(56,134)` found identical saved graphic word `0xc10a`, terrain word `0x200082`, and object word zero. Their separate `candidate_byte` values are respectively `0`, `1`, `72`, and `9`. The four cells are a complete 2×2 within the existing candidate mask and their ID resolves, under the separately documented v213 runtime table, to one 158×95 Type-30 record with a 12,800-byte base. The byte pattern is **not** interpreted as a placement anchor or part index. See [the focused graphics-ID record](graphics-id.md#isolated-multi-cell-saved-id-preview-2026-09-17) for the preview-only grouping rule and the touching Banpo counterexample; neither changes map parsing or the candidate/off-map mask.

## Candidate-byte subtile hypothesis (2026-09-17)

The opt-in `edge-byte` preview interprets the unchanged byte at logical `209471 + y*228 + x` as `part_x=raw&0x07`, `part_y=(raw>>3)&0x07`, marker candidate `raw&0x40`, and still-unknown `raw&0x80`. This is a reference-derived hypothesis using the pinned [Julius `edge_grid` description](https://github.com/bvschaik/julius/blob/34d1ecd54befb845c0139b371fa8a0438210dac1/src/map/property.c), combined with repeated local Emperor patterns. Julius's separate `bitfields_grid` is not this array; its `0x40` must not be conflated. Its `multi_tile_y` masks rather than normalizes, so the C++ profile shifts explicitly. The byte at logical `729311 + index` remains a distinct, uninterpreted auxiliary layer.

The actual Chengdu cells `(55,133),(56,133),(55,134),(56,134)` were reread from `Cities/Chengdu.map`: saved ID `0xc10a`, physical Terrain record 467, bytes `0/0x00/00000000`, `1/0x01/00000001`, `72/0x48/01001000`, `9/0x09/00001001` in that order. Their proposed parts `(0,0),(1,0),(0,1),(1,1)` all yield origin `(55,133)`; only the third has `0x40`. The Banpo eight cells at x=103–104 and y=143–146 all have ID `0x400a1` and physical Elevation record 362. Rows 143–144 repeat bytes `0,1,72,9`, yielding `(103,143)`; rows 145–146 repeat the same bytes and yield `(103,145)`. All have zero unknown high bits. Xia's six and Anyi's fifteen already isolated 2×2 groups show the same part/marker pattern. These are saved-map spatial consistency checks, not proof of a general original-game renderer rule.
