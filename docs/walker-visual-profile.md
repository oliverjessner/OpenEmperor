...
For a selected profile, F3 opens a bounded clip inspection panel over the sandbox. `Q` cycles storage directions, `C`/`E` select the previous/next configured frame without advancing the World, `X` switches 1×/4× nearest-neighbor display, and `B` switches dark/light backgrounds. The panel shows the physical record, frame size, chosen foot anchor, full image boundary, and projected ground point. It intentionally shows the decoded color and alpha as loaded, including any unexplained opaque red pixels. F2 remains the separate live sprite/marker toggle. The panel does not change the simulation's tick-bound animation.
