...
std::size_t building_limit(RulesProfile profile,Object kind) {
    if (!scalable(profile)) {
        ...
        if (kind==Object::ClaySource || kind==Object::Pottery)
            return industry_profile(profile) ? 2U:1U;
        return 1U;
    }
    switch (kind) {
    case Object::Household: return Rules::city_v10_household_limit;
    case Object::ClaySource: return Rules::city_v10_clay_source_limit;
    case Object::Pottery: return Rules::city_v10_pottery_limit;
...
const std::optional<std::vector<Cell>>* dynamic_route(const CourierState& courier,
    ...
    const auto found=std::lower_bound(courier.dynamic_target_routes.begin(),
        courier.dynamic_target_routes.end(),target,
...
const char* rules_profile_name(RulesProfile profile) {
    ...
    case RulesProfile::CityV7: return city_v7_profile_name;
...
World::World(int width,int height,std::vector<std::uint8_t> buildable,RulesProfile profile)
    ...
    buildings_.resize(max_buildings);
    couriers_.resize(max_couriers);
    ...
    couriers_[5].role=CourierRole::Food;
    couriers_[5].owner=BuildingId::Farm;
    couriers_[5].target=BuildingId::Household;
    couriers_[5].good=Good::Food;
    couriers_[6].role=CourierRole::Service;
    couriers_[6].owner=BuildingId::ServicePost;
...
const CourierState& World::courier(CourierId id) const {
    const auto found=std::lower_bound(couriers_.begin(),couriers_.end(),id,
        [](const CourierState& value,CourierId key) { return value.id<key; });
    if (found==couriers_.end() || found->id!=id) throw std::out_of_range("invalid courier ID");
    return *found;
}
...
int World::workforce_supply() const {
    ...
    for (const auto& b:buildings_)
        if (b.placed && b.kind==Object::Household) ++households;
...
int World::workforce_required() const {
    if (!city_profile(profile_)) return 0;
    int total=0;
    for (const auto& b:buildings_) total+=workforce_required(b.id);
    return total;
}
...
bool World::building_staffed_for_tick(BuildingId id) const {
    if (!tick_staffing_active_) return building_staffed(id);
    const auto found=std::lower_bound(buildings_.begin(),buildings_.end(),id,
        [](const BuildingState& value,BuildingId key) { return value.id<key; });
    if (found==buildings_.end() || found->id!=id) throw std::out_of_range("invalid building ID");
    return tick_staffed_[static_cast<std::size_t>(found-buildings_.begin())];
}
...
int World::settlement_goal_households_ready() const {
    if (!city_profile(profile_)) return 0;
    int ready=0;
    for (const auto& home:buildings_) {
...
bool World::settlement_goal_reached() const {
    if (profile_==RulesProfile::CityV10) {
        ...
        return houses>=Rules::city_v10_goal_households &&
            settlement_goal_households_ready()>=Rules::city_v10_goal_level2_households &&
            total_population()>=Rules::city_v10_population_goal;
    }
    return city_profile(profile_) && next_household_id_==household_id_end &&
...
int World::household_population(BuildingId id) const {
    const auto& home=building(id);
    return population_profile(profile_) && home.placed && home.kind==Object::Household ?
        home.population:0;
}

int World::household_population_capacity(BuildingId id) const {
    const auto& home=building(id);
    if (!population_profile(profile_) || !home.placed || home.kind!=Object::Household) return 0;
    const int level=household_level(id);
    return level>=2 ? Rules::household_level2_capacity:
        level==1 ? Rules::household_level1_capacity:Rules::household_level0_capacity;
}

int World::total_population() const {
    if (!population_profile(profile_)) return 0;
    int total=0;
    for (const auto& b:buildings_) if (b.kind==Object::Household)
        total+=household_population(b.id);
    return total;
}

int World::total_population_capacity() const {
    if (!population_profile(profile_)) return 0;
    int total=0;
    for (const auto& b:buildings_) if (b.kind==Object::Household)
        total+=household_population_capacity(b.id);
    return total;
}

int World::unemployed_workers() const {
    return std::max(0,workforce_supply()-workforce_used());
}

bool World::population_valid() const {
    for (const auto& value:buildings_) {
        const auto key=value.id;
        if (!population_profile(profile_)) {
            if (value.population!=0) return false;
            continue;
        }
        if (!value.placed || value.kind!=Object::Household) {
            if (value.population!=0) return false;
            continue;
        }
...
bool World::household_service_active(BuildingId id) const {
    const auto& home=building(id);
    return service_profile(profile_) && home.placed && home.kind==Object::Household &&
        ticks_<home.service_until_tick;
}
...
int World::covered_households() const {
    if (!service_profile(profile_)) return 0;
    int covered=0;
    for (const auto& b:buildings_) if (b.kind==Object::Household && household_service_active(b.id))
        ++covered;
    return covered;
}
...
std::uint64_t World::household_tax_contributed(BuildingId id) const {
    ...
    if (profile_==RulesProfile::CityV6) {
        if (count>UINT64_MAX/static_cast<std::uint64_t>(Rules::tax_income_per_fulfilled_demand))
            throw std::overflow_error("household tax total exhausted");
        return count*static_cast<std::uint64_t>(Rules::tax_income_per_fulfilled_demand);
    }
    if (!food_profile(profile_)) return 0;
    const auto first=std::min<std::uint64_t>(count,1);
    const auto middle=count>1 ? std::min<std::uint64_t>(count-1,3):0;
    const auto high=count>4 ? count-4:0;
    const auto low_rate=static_cast<std::uint64_t>(Rules::city_v7_level0_tax);
    const auto middle_rate=static_cast<std::uint64_t>(Rules::city_v7_level1_tax);
    const auto high_rate=static_cast<std::uint64_t>(Rules::city_v7_level2_tax);
    if (high>(UINT64_MAX-first*low_rate-middle*middle_rate)/high_rate)
        throw std::overflow_error("household tax total exhausted");
    return first*low_rate+middle*middle_rate+high*high_rate;
}
...
CommandResult World::validate(Command command) const {
    ...
    if (command.type==CommandType::PlaceWarehouse && scalable(profile_) &&
        std::count_if(buildings_.begin(),buildings_.end(),[](const BuildingState& b) {
            return b.placed && b.kind==Object::Warehouse;
        })>=static_cast<std::ptrdiff_t>(Rules::city_v10_warehouse_limit)) {
        result.reason="Warehouse limit reached"; return result;
    }
    if (command.type==CommandType::PlaceClaySource || command.type==CommandType::PlacePottery) {
        ...
        if (count>=building_limit(profile_,kind)) {
            result.reason=kind==Object::ClaySource ? "Clay source limit reached":
                "Pottery limit reached"; return result;
        }
        if (!scalable(profile_) && count>0 && (next_production_id_>=10 || next_courier_id_>=6)) {
            result.reason="Production ID limit reached"; return result;
        }
    }
    if (command.type==CommandType::PlaceHousehold &&
        (scalable(profile_) ? std::count_if(buildings_.begin(),buildings_.end(),
            [](const BuildingState& b) { return b.placed && b.kind==Object::Household; })>=
                static_cast<std::ptrdiff_t>(Rules::city_v10_household_limit) :
    ...
    if (command.type==CommandType::PlaceFarm && Rules::farm_cost>treasury_) {
        result.reason="Not enough money"; return result;
    }
    if (command.type==CommandType::PlaceFarm &&
        (scalable(profile_) ? std::count_if(buildings_.begin(),buildings_.end(),
            [](const BuildingState& b) { return b.placed && b.kind==Object::Farm; })>=
                static_cast<std::ptrdiff_t>(Rules::city_v10_farm_limit) :
         building(BuildingId::Farm).placed)) {
        result.reason="Farm limit reached"; return result;
    }
    if (command.type==CommandType::PlaceServicePost &&
        (scalable(profile_) ? std::count_if(buildings_.begin(),buildings_.end(),
            [](const BuildingState& b) { return b.placed && b.kind==Object::ServicePost; })>=
                static_cast<std::ptrdiff_t>(Rules::city_v10_service_post_limit) :
    ...
    if (scalable(profile_) && command.type!=CommandType::PlaceRoad &&
        (next_building_id_==0 || next_building_id_==UINT32_MAX)) {
        result.reason="Building ID counter exhausted"; return result;
    }
...
CommandResult World::execute(Command command) {
    ...
    const auto add_v10_courier=[&](BuildingId owner,CourierRole role,Good good) {
        ...
        courier.id=static_cast<CourierId>(next_courier_id_++);
        courier.role=role; courier.owner=owner; courier.target=owner;
        courier.good=good; courier.enabled=true;
    ...
    case CommandType::PlaceWarehouse:
        ...
        if (scalable(profile_)) {
            ...
            owners_[index(command.cell)]=static_cast<std::uint32_t>(b.id);
        } else warehouse_=command.cell;
        break;
    case CommandType::PlaceClaySource: {
        objects_[index(command.cell)]=Object::ClaySource;
        if (scalable(profile_)) {
            auto& b=add_v10_building(Object::ClaySource);
            b.placed_tick=ticks_;
            add_v10_courier(b.id,CourierRole::Clay,Good::Clay);
            break;
        }
    ...
    case CommandType::PlacePottery: {
        ...
        if (scalable(profile_)) {
            auto& b=add_v10_building(Object::Pottery);
            b.placed_tick=ticks_;
            add_v10_courier(b.id,CourierRole::Pottery,Good::Pottery);
            break;
        }
    ...
    case CommandType::PlaceHousehold: {
        ...
        if (scalable(profile_)) {
            auto& b=add_v10_building(Object::Household);
            b.placed_tick=ticks_; b.population=Rules::household_initial_population;
            break;
        }
        ...
        if (population_profile(profile_)) b.population=Rules::household_initial_population;
        owners_[index(command.cell)]=static_cast<std::uint32_t>(b.id);
        mutable_courier(CourierId::Household).enabled=true;
        ++next_household_id_;
        break;
    }
    case CommandType::PlaceFarm: {
        objects_[index(command.cell)]=Object::Farm;
        if (scalable(profile_)) {
    ...
    case CommandType::PlaceServicePost: {
        objects_[index(command.cell)]=Object::ServicePost;
        if (scalable(profile_)) {
            auto& post=add_v10_building(Object::ServicePost);
            post.placed_tick=ticks_;
            add_v10_courier(post.id,CourierRole::Service,Good::Goods);
            break;
        }
        auto& post=mutable_building(BuildingId::ServicePost);
        post.placed=true; post.kind=Object::ServicePost; post.cell=command.cell;
        post.placed_tick=ticks_;
...
void World::refresh_routes() {
    ...
    if (scalable(profile_)) {
        ...
        household_routes_revision_=food_routes_revision_=service_routes_revision_=road_revision_;
        return;
    }
    for (auto& courier:couriers_) {
        const auto& source=building(courier.owner);
    ...
    if (food_profile(profile_)) {
        const auto& farm=building(BuildingId::Farm);
        for (std::size_t i=0;i<food_household_routes_.size();++i) {
            const auto& home=building(static_cast<BuildingId>(first_household_id+i));
            food_household_routes_[i]=farm.placed && home.placed ?
                find_route(farm.cell,home.cell):std::nullopt;
        }
        food_routes_revision_=road_revision_;
    }
    if (service_profile(profile_)) {
        const auto& post=building(BuildingId::ServicePost);
        for (std::size_t i=0;i<service_household_routes_.size();++i) {
            const auto& home=building(static_cast<BuildingId>(first_household_id+i));
            service_household_routes_[i]=post.placed && home.placed ?
                find_route(post.cell,home.cell):std::nullopt;
        }
        service_routes_revision_=road_revision_;
    }
}

std::size_t World::route_cache_entries() const {
    if (scalable(profile_)) {
        std::size_t total=0;
        for (const auto& c:couriers_) total+=c.dynamic_target_routes.size();
        return total;
    }
    std::size_t total=0;
    for (const auto& c:couriers_) {
        total+=c.cached_route.has_value();
        total+=static_cast<std::size_t>(std::count_if(c.target_routes.begin(),c.target_routes.end(),
            [](const auto& route) { return route.has_value(); }));
    }
    total+=static_cast<std::size_t>(std::count_if(household_routes_.begin(),household_routes_.end(),
        [](const auto& route) { return route.has_value(); }));
    total+=static_cast<std::size_t>(std::count_if(food_household_routes_.begin(),food_household_routes_.end(),
        [](const auto& route) { return route.has_value(); }));
    total+=static_cast<std::size_t>(std::count_if(service_household_routes_.begin(),service_household_routes_.end(),
        [](const auto& route) { return route.has_value(); }));
    return total;
}
...
CourierDispatchDecision World::courier_dispatch_status(CourierId id) const {
    ...
    if (c.cached_revision!=road_revision_ ||
        ((c.role==CourierRole::Household &&
         ...
         household_routes_revision_!=road_revision_) ||
         (c.role==CourierRole::Food && food_routes_revision_!=road_revision_) ||
         (c.role==CourierRole::Service && service_routes_revision_!=road_revision_)))
    ...
    if (scalable(profile_)) {
        ...
        if (c.last_dispatched_target) {
            const auto found=std::find_if(targets.begin(),targets.end(),[&](const auto* b) {
                return b->id==*c.last_dispatched_target;
            });
        ...
        for (std::size_t step=0;step<targets.size();++step) {
            ...
            if (route && *route) return {CourierDispatchStatus::Ready,target.id};
        }
        return {any_free ? CourierDispatchStatus::NoRoad:CourierDispatchStatus::TargetFull,
                std::nullopt};
    }

    if (c.role==CourierRole::Service && service_profile(profile_)) {
        if (next_household_id_==first_household_id)
            return {CourierDispatchStatus::NoTarget,std::nullopt};
        const unsigned start=last_dispatched_service_household_ ?
            static_cast<unsigned>(*last_dispatched_service_household_):household_id_end-1U;
        ...
        for (unsigned step=1;step<=household_limit;++step) {
            ...
            any_target=true;
            if (service_household_routes_[value-first_household_id])
                return {CourierDispatchStatus::Ready,candidate};
        }
        return {any_target ? CourierDispatchStatus::NoRoad:CourierDispatchStatus::NoTarget,
                std::nullopt};
    }
    const int source_stock=c.role==CourierRole::Household ? source.pottery_stock:source.output;
    ...
    if (c.role==CourierRole::Food && food_profile(profile_)) {
        if (next_household_id_==first_household_id)
            return {CourierDispatchStatus::NoTarget,std::nullopt};
        const unsigned start=last_dispatched_food_household_ ?
            static_cast<unsigned>(*last_dispatched_food_household_):household_id_end-1U;
        bool any_free=false;
        for (unsigned step=1;step<=household_limit;++step) {
            const unsigned value=first_household_id+
                (start-first_household_id+step)%household_limit;
            ...
            const auto& target=building(candidate);
            if (!target.placed || target.kind!=Object::Household) continue;
            if (target.food_stock+target.reserved_food_incoming>=Rules::household_food_capacity)
                continue;
            any_free=true;
            if (food_household_routes_[value-first_household_id])
                return {CourierDispatchStatus::Ready,candidate};
        }
        return {any_free ? CourierDispatchStatus::NoRoad:CourierDispatchStatus::TargetFull,
                std::nullopt};
    ...
    const int target_stock=c.role==CourierRole::Clay ? target.input_clay:
        c.role==CourierRole::Food ? target.food_stock:target.pottery_stock;
    const int capacity=c.role==CourierRole::Clay ? Rules::pottery_input_capacity :
        c.role==CourierRole::Pottery ? Rules::warehouse_capacity:
        c.role==CourierRole::Food ? Rules::household_food_capacity:Rules::household_capacity;
    const int reserved=c.role==CourierRole::Food ? target.reserved_food_incoming:
        target.reserved_incoming;
    if (target_stock+reserved>=capacity)
...
void World::tick_production_v2() {
    ...
    if (scalable(profile_)) {
        for (auto& farm:buildings_) if (farm.placed && farm.kind==Object::Farm &&
            ...
            } else farm.progress=0;
        }
    } else if (food_profile(profile_)) {
        auto& farm=mutable_building(BuildingId::Farm);
        if (farm.placed && building_staffed_for_tick(farm.id)) {
            if (farm.output<Rules::farm_output_capacity) {
                if (++farm.progress==Rules::farm_ticks) {
                    ...
                    ++farm.output;
                    ++farm.food_produced;
                    ++food_produced_total_;
                }
            } else farm.progress=0;
        }
    }
    if (household_profile(profile_)) for (auto& home:buildings_) {
        if (home.kind!=Object::Household) continue;
        if (home.placed) {
...
void World::dispatch_v2(CourierState& courier) {
    ...
    const bool cycle_food=courier.role==CourierRole::Food && food_profile(profile_);
    const bool cycle_service=courier.role==CourierRole::Service && service_profile(profile_);
    ...
    const auto selected_food_home=cycle_food ? decision.selected_target:std::nullopt;
    const auto selected_service_home=cycle_service ? decision.selected_target:std::nullopt;
    ...
    if (scalable(profile_)) {
        const auto* cached=dynamic_route(courier,target_id);
        if (cached && *cached) route=&**cached;
    } else if (selected_service_home) {
        const auto& cached=service_household_routes_[
            static_cast<unsigned>(*selected_service_home)-first_household_id];
        if (cached) route=&*cached;
    } else if (selected_food_home) {
        const auto& cached=food_household_routes_[
            static_cast<unsigned>(*selected_food_home)-first_household_id];
        if (cached) route=&*cached;
    } else if (selected_home) {
    ...
    if (courier.role==CourierRole::Service && scalable(profile_)) {
        ...
        courier.last_dispatched_target=target_id;
        courier.path_vertex=0; courier.edge_progress=0; courier.route_pending=false;
        courier.route_checked_revision.reset(); courier.phase=CourierPhase::ToWarehouse;
        return;
    }
    if (selected_service_home) {
        if (source.kind!=Object::ServicePost || target.kind!=Object::Household ||
            courier.good!=Good::Goods || courier.cargo || courier.reserved)
            throw std::logic_error("invalid Service visit relation");
        courier.path=*route;
        courier.target=*selected_service_home;
        last_dispatched_service_household_=*selected_service_home;
    ...
    int* destination_reservation=&target.reserved_incoming;
    ...
    case CourierRole::Food:
        if (!food_profile(profile_) || source.kind!=Object::Farm ||
            target.kind!=Object::Household || courier.good!=Good::Food)
            throw std::logic_error("invalid Food delivery relation");
    ...
    *destination_reservation+=amount;
    if (selected_food_home) {
        courier.target=*selected_food_home;
        if (!scalable(profile_)) last_dispatched_food_household_=*selected_food_home;
    } else if (selected_home) {
        courier.target=*selected_home;
        if (!scalable(profile_)) last_dispatched_household_=*selected_home;
    } else if (selected_pottery) {
        courier.target=*selected_pottery;
        if (!scalable(profile_)) courier.last_dispatched_pottery=*selected_pottery;
    }
    if (scalable(profile_)) {
        courier.target=target_id;
        courier.last_dispatched_target=target_id;
    }
    courier.path_vertex=0;
    courier.edge_progress=0;
...
void World::move_v2(CourierState& courier) {
    ...
    if (courier.phase==CourierPhase::ToWarehouse) {
        ...
        if (courier.role==CourierRole::Service) {
            ...
            if (ticks_>UINT64_MAX-Rules::service_coverage_ticks)
...
const char* World::pottery_blockage() const {
    ...
    if (city_profile(profile_) && !building_staffed(b.id)) return "Building unstaffed";
...
bool World::industry_balance_valid() const {
    if (scalable(profile_)) {
        ...
        for (std::size_t i=0;i<buildings_.size();++i) {
            ...
            case Object::Warehouse:
                ...
                if (b.pottery_stock<0 || b.pottery_stock+b.reserved_incoming>Rules::warehouse_capacity)
                    return false;
                pottery+=static_cast<std::uint64_t>(b.pottery_stock); break;
        ...
        for (std::size_t i=0;i<couriers_.size();++i) {
            ...
            if (!c.enabled || static_cast<std::uint32_t>(c.id)==0 ||
                (i && !(couriers_[i-1].id<c.id)) || c.cargo<0 || c.cargo>Rules::courier_capacity ||
                c.reserved<0 || c.reserved>Rules::courier_capacity ||
                c.reserved!=(c.phase==CourierPhase::ToWarehouse ? c.cargo:0)) return false;
        ...
        return extracted==clay_extracted_total_ && recipes==pottery_completed_total_ &&
            ...
            clay==clay_extracted_total_ &&
            pottery+consumed==pottery_completed_total_ && food+consumed_food==food_produced_total_;
    }
    ...
    std::array<int,legacy_max_buildings> expected_reservations{};
    for (unsigned id=1;id<=legacy_max_buildings;++id) {
        ...
        if (!b.placed) {
            if (b.kind!=Object::Empty || b.cell!=Cell{} || b.input_clay || b.output ||
                ...
                b.last_demand_status || b.clay_extracted || b.food_stock ||
        ...
        if (b.kind==Object::ClaySource) {
            ...
            if (id<4 || id>=next_household_id_ ||
                ...
                b.consumed_total!=b.fulfilled_demand ||
                (!service_profile(profile_) && b.service_until_tick!=0) ||
                (service_profile(profile_) && b.service_until_tick!=0 &&
                 b.service_until_tick>ticks_ &&
                 b.service_until_tick-ticks_>Rules::service_coverage_ticks) ||
    ...
    for (unsigned id=1;id<=legacy_max_couriers;++id) {
    ...
    return food_balance_valid();
}

bool World::food_balance_valid() const {
    if (scalable(profile_)) return true; // Covered by the unified City-v10 balance audit.
    const auto& farm=building(BuildingId::Farm);
    const auto& food=courier(CourierId::Food);
    if (!food_profile(profile_)) {
        if (food_produced_total_ || last_dispatched_food_household_ ||
            farm!=BuildingState{BuildingId::Farm}) return false;
        CourierState empty;
        empty.id=CourierId::Food; empty.role=CourierRole::Food;
        empty.owner=BuildingId::Farm; empty.target=BuildingId::Household; empty.good=Good::Food;
        if (food.id!=empty.id || food.role!=empty.role || food.owner!=empty.owner ||
            food.target!=empty.target || food.good!=empty.good || food.enabled ||
            food.phase!=CourierPhase::IdleAtWorkshop || food.cargo || food.reserved ||
            !food.path.empty() || food.path_vertex || food.edge_progress || food.route_pending ||
            food.route_checked_revision || food.reroute_attempts) return false;
        for (const auto& b:buildings_)
            if (b.food_stock || b.reserved_food_incoming || b.food_consumed_total ||
                b.food_produced) return false;
        return true;
    }
    if (farm.id!=BuildingId::Farm || food.id!=CourierId::Food ||
        food.role!=CourierRole::Food || food.owner!=BuildingId::Farm ||
        food.good!=Good::Food || food.enabled!=farm.placed ||
        farm.food_produced!=food_produced_total_) return false;
    if (farm.placed) {
        if (farm.kind!=Object::Farm || !in_bounds(farm.cell) ||
            object_at(farm.cell)!=Object::Farm || building_owner_at(farm.cell)!=BuildingId::Farm ||
            farm.output<0 || farm.output>Rules::farm_output_capacity ||
            farm.progress<0 || farm.progress>=Rules::farm_ticks ||
            (farm.output==Rules::farm_output_capacity && farm.progress!=0) ||
            farm.placed_tick>ticks_ || farm.food_produced>(ticks_-farm.placed_tick)/Rules::farm_ticks ||
            farm.input_clay || farm.pottery_stock || farm.reserved_incoming ||
            farm.active_recipe_clay || farm.recipes_completed || farm.demand_progress ||
            farm.fulfilled_demand || farm.missed_demand || farm.consumed_total ||
            farm.last_demand_status || farm.clay_extracted || farm.food_stock ||
            farm.reserved_food_incoming || farm.food_consumed_total ||
            farm.service_until_tick || farm.population) return false;
    } else if (farm!=BuildingState{BuildingId::Farm}) return false;
    if (food.cargo<0 || food.cargo>Rules::courier_capacity ||
        food.reserved<0 || food.reserved>Rules::courier_capacity ||
        food.reserved!=(food.phase==CourierPhase::ToWarehouse ? food.cargo:0) ||
        (food.phase==CourierPhase::ToWarehouse && food.cargo==0) ||
        (!food.enabled && (food.phase!=CourierPhase::IdleAtWorkshop || food.cargo || food.reserved)))
        return false;
    std::uint64_t total=static_cast<std::uint64_t>(farm.output+food.cargo);
    int reserved=0;
    for (unsigned id=first_household_id;id<household_id_end;++id) {
        const auto& home=building(static_cast<BuildingId>(id));
        if (home.food_stock<0 || home.food_stock>Rules::household_food_capacity ||
            home.reserved_food_incoming<0 ||
            home.food_stock+home.reserved_food_incoming>Rules::household_food_capacity ||
            home.food_consumed_total!=home.fulfilled_demand ||
            (!home.placed && (home.food_stock || home.reserved_food_incoming ||
                              home.food_consumed_total || home.food_produced)) ||
            home.food_produced || total>UINT64_MAX-static_cast<std::uint64_t>(home.food_stock) ||
            total+static_cast<std::uint64_t>(home.food_stock)>
                UINT64_MAX-home.food_consumed_total) return false;
        total+=static_cast<std::uint64_t>(home.food_stock)+home.food_consumed_total;
        reserved+=home.reserved_food_incoming;
        if (home.reserved_food_incoming>0 &&
            (food.phase!=CourierPhase::ToWarehouse || food.target!=static_cast<BuildingId>(id)))
            return false;
    }
    for (unsigned id=1;id<=max_buildings;++id) {
        if (id>=first_household_id && id<household_id_end) continue;
        const auto& b=building(static_cast<BuildingId>(id));
        if (id!=static_cast<unsigned>(BuildingId::Farm) &&
            (b.food_stock || b.reserved_food_incoming || b.food_consumed_total || b.food_produced))
            return false;
    }
    if (reserved!=food.reserved || total!=food_produced_total_) return false;
    if (last_dispatched_food_household_ &&
        (static_cast<unsigned>(*last_dispatched_food_household_)<first_household_id ||
         static_cast<unsigned>(*last_dispatched_food_household_)>=next_household_id_)) return false;
    return true;
}

bool World::service_state_valid() const {
    if (scalable(profile_)) {
        for (const auto& b:buildings_)
            if (b.kind!=Object::Household && b.service_until_tick) return false;
        for (const auto& c:couriers_) if (c.role==CourierRole::Service &&
            (c.cargo || c.reserved || building(c.owner).kind!=Object::ServicePost)) return false;
        return true;
    }
    const auto& post=building(BuildingId::ServicePost);
    const auto& service=courier(CourierId::Service);
    CourierState empty;
    empty.id=CourierId::Service;
    empty.role=CourierRole::Service;
    empty.owner=BuildingId::ServicePost;
    empty.target=BuildingId::Household;
    empty.good=Good::Goods;
    if (!service_profile(profile_)) {
        if (last_dispatched_service_household_ || post!=BuildingState{BuildingId::ServicePost})
            return false;
        if (service.id!=empty.id || service.role!=empty.role || service.owner!=empty.owner ||
            service.target!=empty.target || service.good!=empty.good || service.enabled ||
            service.phase!=CourierPhase::IdleAtWorkshop || service.cargo || service.reserved ||
            !service.path.empty() || service.path_vertex || service.edge_progress ||
            service.route_pending || service.route_checked_revision || service.reroute_attempts ||
            service.last_dispatched_pottery) return false;
        for (const auto& b:buildings_) if (b.service_until_tick) return false;
        return true;
    }
    if (post.id!=BuildingId::ServicePost || service.id!=CourierId::Service ||
        service.role!=CourierRole::Service || service.owner!=BuildingId::ServicePost ||
        service.good!=Good::Goods || service.enabled!=post.placed ||
        service.cargo || service.reserved || service.last_dispatched_pottery) return false;
    if (service_routes_revision_!=std::numeric_limits<std::uint64_t>::max() &&
        service_routes_revision_!=road_revision_) return false;
    if (post.placed) {
        if (post.kind!=Object::ServicePost || !in_bounds(post.cell) ||
            object_at(post.cell)!=Object::ServicePost ||
            building_owner_at(post.cell)!=BuildingId::ServicePost || post.placed_tick>ticks_ ||
            post.input_clay || post.output || post.pottery_stock || post.reserved_incoming ||
            post.progress || post.active_recipe_clay || post.recipes_completed ||
            post.demand_progress || post.fulfilled_demand || post.missed_demand ||
            post.consumed_total || post.last_demand_status || post.clay_extracted ||
            post.food_stock || post.reserved_food_incoming || post.food_consumed_total ||
            post.food_produced || post.service_until_tick || post.population) return false;
    } else if (post!=BuildingState{BuildingId::ServicePost}) return false;
    if (!service.enabled && (service.phase!=CourierPhase::IdleAtWorkshop ||
        !service.path.empty() || service.path_vertex || service.edge_progress ||
        service.route_pending || service.route_checked_revision || service.reroute_attempts))
        return false;
    if (service.phase!=CourierPhase::IdleAtWorkshop) {
        const auto value=static_cast<unsigned>(service.target);
        if (value<first_household_id || value>=next_household_id_ ||
            building(service.target).kind!=Object::Household) return false;
    }
    if (last_dispatched_service_household_) {
        const auto value=static_cast<unsigned>(*last_dispatched_service_household_);
        if (value<first_household_id || value>=next_household_id_ ||
            !building(*last_dispatched_service_household_).placed) return false;
    }
    for (unsigned id=1;id<=max_buildings;++id) {
        const auto& b=building(static_cast<BuildingId>(id));
        const bool home=id>=first_household_id && id<household_id_end;
        if (!home && b.service_until_tick) return false;
        if (home && b.service_until_tick>ticks_ &&
            b.service_until_tick-ticks_>Rules::service_coverage_ticks) return false;
    }
    return true;
}
...
std::string World::canonical_state() const {
    ...
    out<<static_cast<int>(profile_)<<':'<<ticks_<<':'<<command_sequence_<<':'<<road_revision_
       ...
       <<static_cast<unsigned>(next_courier_id_)<<':'<<next_building_id_<<':'
       <<static_cast<unsigned>(last_dispatched_household_.value_or(static_cast<BuildingId>(0)))
       <<':'<<static_cast<unsigned>(last_dispatched_food_household_.value_or(static_cast<BuildingId>(0)))
       <<':'<<static_cast<unsigned>(last_dispatched_service_household_.value_or(static_cast<BuildingId>(0)))
    ...
    for (const auto& c:couriers_) {
        out<<'|'<<static_cast<int>(c.id)<<','<<static_cast<int>(c.role)<<','
           ...
           <<static_cast<std::uint32_t>(c.last_dispatched_target.value_or(
                static_cast<BuildingId>(0)))<<','
...
WorldSnapshot World::snapshot() const {
    ...
    s.next_building_id=next_building_id_;
    s.last_dispatched_household=last_dispatched_household_;
    s.last_dispatched_food_household=last_dispatched_food_household_;
    s.last_dispatched_service_household=last_dispatched_service_household_;
    s.food_produced_total=food_produced_total_;
    ...
    for (std::size_t i=0;i<buildings_.size();++i) {
        const auto& b=buildings_[i];
        s.buildings.push_back({b.id,b.kind,b.cell,b.placed,b.input_clay,b.output,b.pottery_stock,
    ...
    for (std::size_t i=0;i<couriers_.size();++i) {
        const auto& c=couriers_[i];
        s.couriers.push_back({c.id,c.role,c.owner,c.target,c.good,c.enabled,c.phase,c.cargo,c.reserved,
...
World World::restore(const WorldSnapshot& s,std::vector<std::uint8_t> mask) {
    ...
    const auto put=[&](Cell cell,Object object,BuildingId owner) {
        ...
        w.objects_[i]=object;
        if (object!=Object::Road) w.owners_[i]=static_cast<std::uint32_t>(owner);
    };
    ...
    w.next_building_id_=s.next_building_id;
    w.last_dispatched_household_=s.last_dispatched_household;
    w.last_dispatched_food_household_=s.last_dispatched_food_household;
    ...
    if (scalable(s.profile)) {
        ...
        if ((!s.buildings.empty() && s.next_building_id<=
            ...
            (!s.couriers.empty() && s.next_courier_id<=
             static_cast<std::uint32_t>(s.couriers.back().id)))
            fail("City-v10 next ID does not exceed existing IDs");
        for (std::size_t i=0;i<s.couriers.size();++i) {
            const auto& c=s.couriers[i];
            if (static_cast<std::uint32_t>(c.id)==0 || (i && !(s.couriers[i-1].id<c.id)) ||
                !c.enabled) fail("invalid City-v10 courier identity");
            try { (void)w.building(c.owner); (void)w.building(c.target); }
            catch (const std::out_of_range&) { fail("City-v10 courier references missing building"); }
            CourierState out;
            out.id=c.id; out.role=c.role; out.owner=c.owner; out.target=c.target;
            out.good=c.good; out.enabled=c.enabled; out.phase=c.phase; out.cargo=c.cargo;
            out.reserved=c.reserved; out.path=c.path; out.path_vertex=c.path_vertex;
            out.edge_progress=c.edge_progress; out.route_pending=c.route_pending;
            ...
            out.last_dispatched_target=c.last_dispatched_target;
            if (c.last_dispatched_target) {
    ...
    if (industry_profile(s.profile)) {
        ...
        for (std::size_t i=0;i<s.couriers.size();++i) {
            ...
            if (c.id!=static_cast<CourierId>(i+1) ||
                static_cast<unsigned>(c.owner)<1 || static_cast<unsigned>(c.owner)>max_buildings ||
                static_cast<unsigned>(c.target)<1 || static_cast<unsigned>(c.target)>max_buildings)
    ...
    for (std::size_t i=3;i<s.couriers.size();++i) {
        ...
        if (i==5) {
            empty.role=CourierRole::Food;
            empty.target=BuildingId::Household;
            empty.good=Good::Food;
        } else if (i==6) {
            empty.role=CourierRole::Service;
            empty.owner=BuildingId::ServicePost;
            empty.target=BuildingId::Household;
