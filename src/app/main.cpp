...
void print_usage(const char* executable) {
    ...
    std::cerr << "       " << executable << " --data <directory> --sandbox <relative.map>"
              << " [--sandbox-rules sandbox-logistics-v1|sandbox-production-v2|sandbox-household-v3|sandbox-settlement-v4|sandbox-industry-v5|sandbox-city-v6|sandbox-city-v7|sandbox-city-v8]"
...
int main(int argc, char* argv[]) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (argument == "--browse-assets" && !browse_assets) {
            ...
            else if (value==openemperor::simulation::city_v8_profile_name)
