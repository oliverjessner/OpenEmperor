...
- The reusable `src/assets/` loader reads only one selected `.555` payload into memory, without SDL or an inspector-tool dependency. Its supported paths are documented plain RGB555, Omega Sprite types 256/257/276, and Type-30 isometric base with optional Omega overlay in classic 58×30 or Emperor 78×40 geometry. The app renders the resulting RGBA image directly in SDL3; PNG debugging preview and inspector export remain separate options. Reject unsafe external paths, out-of-bounds ranges, alpha masks, and unsupported layouts. Keep `horizontal_mirror_offset` parsed but unapplied until verified; do not invent semantics or undocumented offset fixes. Do not import original or decoded assets into the repository or add game logic without a later scoped request.
