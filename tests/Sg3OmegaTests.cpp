...
bool run_checks() {
    ...
    if (decode_omega_color_rgba(Bytes{1, 0x00, 0x7c}, 1, 1).pixels != pixels({red})) return false;
    if (decode_omega_color_rgba(Bytes{2, 0x00, 0x7c, 0xe0, 0x03}, 2, 1).pixels !=
        pixels({red, green})) return false;
    if (decode_omega_color_rgba(Bytes{255, 1, 1, 0x1f, 0x00}, 3, 1).pixels !=
        pixels({transparent, blue, transparent})) return false;
    if (decode_omega_color_rgba(Bytes{255, 1, 2, 0x00, 0x7c, 0xe0, 0x03}, 2, 2).pixels !=
        pixels({transparent, red, green, transparent})) return false;
    if (decode_omega_color_rgba(Bytes{1, 0x00, 0x7c, 255, 2, 2,
                                      0xe0, 0x03, 0x1f, 0x00}, 3, 2).pixels !=
        pixels({red, transparent, transparent, green, blue, transparent})) return false;
    if (decode_omega_color_rgba(Bytes{1, 0x23, 0x4a}, 1, 1).pixels !=
        Bytes{148, 140, 24, 255}) return false;
    ...
    if (!rejects([&] { decode_omega_color_rgba(Bytes{255}, 1, 1); }) ||
        !rejects([&] { decode_omega_color_rgba(Bytes{2, 0x00, 0x7c}, 2, 1); }) ||
        !rejects([&] { decode_omega_color_rgba(Bytes{255, 2}, 1, 1); }) ||
        !rejects([&] { decode_omega_color_rgba(Bytes{2, 0x00, 0x7c, 0xe0, 0x03}, 1, 1); }) ||
        !rejects([&] { decode_omega_color_rgba(Bytes{1, 0x00, 0x7c, 0}, 1, 1); }) ||
